(function ($) {
  ...
  $('a[data-toggle="tab"]').on("shown.bs.tab", function (e) {
    ...
    var targetChild = $(target).children().find(".ft-user");
    var relatedTargetChild = $(relatedTarget).children().find(".ft-user");
  ...
  $(".user-view-mode a").on("click", function (e) {
    e.preventDefault();

    var $this = $(this);
    var shopUserWrap = $(".shop-users");
    var viewMode = $this.data("target");

    $(".user-view-mode a").removeClass("active");
    $this.addClass("active");
    shopUserWrap.removeClass("grid list").addClass(viewMode);

    if ($this.parents(".shop-page-wrapper").hasClass("shop-fullwidth")) {
      if (viewMode === "list") {
        shopUserWrap.removeClass("container-fluid");
        shopUserWrap.addClass("container");
      } else {
        if (shopUserWrap.hasClass("container")) {
          shopUserWrap.removeClass("container");
          shopUserWrap.addClass("container-fluid");
        }
      }
    }
  });
  ...
  var userThumb = $(".user-gallery__image img"),
    imageSrcLength = userThumb.length,
    images = [],
    indexNumbArray = [];

  for (var i = 0; i < userThumb.length; i++) {
    images[i] = { src: userThumb[i].src };
  }
